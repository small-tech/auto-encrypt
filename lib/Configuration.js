...
class Configuration {
   ...
   */
  static initialise (settings = throws.ifMissing()) {

    // Additional argument validation.
    throws.if(this.#initialised, Symbol.for('Configuration.alreadyInitialisedError'))
   ...
   */
  static get domains () {
    return this.valueIfInitialised(this.#domains, 'domains')
  }
  ...
  static get settingsPath () {
    return this.valueIfInitialised(this.#settingsPath, 'settingsPath')
  }
  ...
  static get accountPath () {
    return this.valueIfInitialised(this.#accountPath, 'accountPath')
  }
   ...
   * @readonly
   */
  static get accountIdentityPath () {
    return this.valueIfInitialised(this.#accountIdentityPath, 'accountIdentityPath')
  }
  ...
  static get certificatePath () {
    return this.valueIfInitialised(this.#certificatePath, 'certificatePath')
  }
  ...
  /**
   * Helper method: returns the value if the Configuration static class has been initialised. Otherwise throws.
   *
   * @static
   * @param {*}      value The value to return (may be null/undefined)
   * @param {String} name  The key/property that the value corresponds to. Used in the error message.
   * @returns {*}
   */
  static valueIfInitialised (value, name) {
    if (!this.#initialised) {
      throws.error(Symbol.for('Configuration.notInitialisedError'), name)
    }
    return value
  }
  ...
  static throwReadOnlyAccessorError (setterName) {
    throws.error(Symbol.for('ReadOnlyAccessorError'), setterName, 'All static Configuration accessors are read-only. Call Configuration.initialise() to mutate the configuration.')
  }
   ...
   */
  constructor () {
    throws.error(Symbol.for('StaticClassCannotBeInstantiatedError'), 'Configuration is a static class. Call Configuration.initialise() to set it up.')
  }
}
