...
class Configuration {
   ...
   * @param {LetsEncryptServer} settings.letsEncryptServer Let’s Encrypt Server to use.
  ...
  constructor (settings = throws.ifMissing()) {
    ...
    throws.ifUndefinedOrNull(settings.letsEncryptServer, 'settings.letsEncryptServer')
    ...
    this.#letsEncryptServer = settings.letsEncryptServer
    ...
    if (settings.settingsPath === null) {
      ...
      this.#settingsPath = defaultPathFor(this.#letsEncryptServer.name)
    } else {
      // Create the path to use based on the custom root path we’ve been passed.
      this.#settingsPath = path.join(settings.settingsPath, this.#letsEncryptServer.name)
    }
   ...
   * @readonly
   */
  get letsEncryptServer () {
    console.log('returning', this.#letsEncryptServer)
    return this.#letsEncryptServer
  }
  ...
  set letsEncryptServer        (state) { this.throwReadOnlyAccessorError('letsEncryptServer')        }
  ...
  [util.inspect.custom] () {
    return `
      ...
      Using ${this.letsEncryptServer.name} server paths.

      Property                   Description                             Value
      -------------------------- --------------------------------------- ---------------------------------------
      .letsEncryptServer       : Lets Encrypt Server details             {name: '${this.letsEncryptServer.name}', endpoint: …}
