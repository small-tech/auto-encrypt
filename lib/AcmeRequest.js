...
class AcmeRequest {
  ...
  ERRORS = {
    // NB. Global errors are mixed into this object.
    [Symbol.for('AcmeRequest.requestError')]: errorBody => errorBody
  }

  constructor () {
    mixSaferAndDRYerErrorHandlingInto(this)
  }
   ...
   * @returns {types.ResponseObject}
   */
  async _execute (preparedRequest = this.required(), parseResponseBodyAsJSON = this.required()) {
    ...
    if (errorBody) {
      ...
      this.throwError(Symbol.for('AcmeRequest.requestError', errorMessage))
    }
