...
class AcmeRequest {
  ...
  async execute (command, payload, useKid=true, successCodes=[200]) {
    return await this._execute (await this.prepare(command, payload, useKid, /* url = */ null, successCodes))
  }
  ...
  // to our expectations.
  async prepare (command, payload, useKid, url = null, successCodes = [200]) {
