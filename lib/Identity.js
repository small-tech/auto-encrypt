...
class Identity {
  ...
  static isBeingInstantiatedViaSubclassFactoryMethod = false

  constructor (identityFilePath) {
    ...
    if (Identity.isBeingInstantiatedViaSubclassFactoryMethod === false) {
      throw new Error('Identity is an abstract base class and cannot be instantiated directly. Please instantiate using either AccountIdentity.getSharedInstance() (singleton) or CertificateIdentity.getInstance()')
    }
    Identity.isBeingInstantiatedViaSubclassFactoryMethod = false

    const uniqueIdentifier = identityFilePath.slice(identityFilePath.indexOf(`/${Configuration.staging ? 'staging' : 'production'}/`),)
    console.log(` 👤 Creating identity for ${uniqueIdentifier}`)
