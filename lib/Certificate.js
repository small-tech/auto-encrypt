...
class Certificate {
  ...
  attemptToRecoverFromFailedRenewalAttemptIfNecessary () {
    ...
    const oldCertificatePath = `${this.#configuration.certificateIdentityPath}.old`
  ...
  async createSecureContext (renewCertificate = false) {
    ...
    if (!renewCertificate) {
      ...
      this.#account = Account.getInstanceAsync(this.#configuration)
      this.#directory = Directory.getInstanceAsync(this.#configuration)
      this.#accountIdentity = new AccountIdentity(this.#configuration)

      AcmeRequest.initialise(this.#directory, this.#account, this.#accountIdentity)
    }
