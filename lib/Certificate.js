...
class Certificate {
  constructor (domains) {
    ...
    if (domains == undefined || !Array.isArray(domains) || domains.length === 0 || domains.length > 100) {
      this.throwError(Symbol.for('ArgumentError', 'domains', 'must be an array of 1-100 strings'))
    }
  ...
  set pem (certificatePem) {
    ...
    log(` 📆 Certificate set. Issued ${this.issueDate.calendar().toLowerCase()} (${this.issueDate.fromNow()}) and expires ${this.expiryDate.calendar().toLowerCase()} (${this.expiryDate.fromNow()}).`)
  }
  ...
  set key        (value) { this.throwError(Symbol.for('ReadOnlyAccessorError'), 'key', 'is derived from identity')  }
  set issueDate  (value) { this.throwError(Symbol.for('ReadOnlyAccessorError'), 'issueDate', 'is derived from pem') }
  set expiryDate (value) { this.throwError(Symbol.for('ReadOnlyAccessorError'), 'issueDate', 'is derived from pem') }
   ...
   * @returns {Promise}
   * @fulfil {(no value)} Fulfils immediately if certificate exists and does not need to be renewed; otherwise fulfils when certificate has been provisioned.
   */
   ...
   * @category async
   * @returns {Promise}
   * @fulfil {(no value)} Fulfils once a certificate has been provisioned.
   */
   ...
   * @access private
   * @returns {Promise}
   * @fulfil {(no value)} Resolves once certificate is renewed and new secure context created and cached.
   * @category async
   */
   ...
   * @category async
   * @returns {Promise}
   * @fulfil {(no value)} Either immediately if certificate doesn’t need renewal or once certificate has been renewed.
   */
