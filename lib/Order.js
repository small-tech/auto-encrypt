...
class Order {
  ...
  async init () {
    ...
    this._certificateIdentity = CertificateIdentity.getInstance()
    ...
    if (this.status === 'valid') {
      ...
      try {
        await fs.writeFile(Configuration.certificatePath, this.certificate, 'utf-8')
      } catch (error) {
        throw new Error(error)
      }

      console.log(' 💾 Saved the certificate.')
    } else {
